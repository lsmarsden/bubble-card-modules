...
describe("resolveColor()", () => {
  ...
  it("returns default color if no resolved state", () => {
    hass.getState.mockReturnValue(undefined);
    expect(resolveColor("sensor.unknown", "rgba(255,255,255,0.4)")).toBe("rgba(255,255,255,0.4)");
  });
...
describe("resolveColorFromStops()", () => {
  ...
  describe.each([true, false])("shared behavior (interpolate = %s)", (interpolate) => {
    it("returns default when stops is null or empty", () => {
      expect(resolveColorFromStops(50, null, interpolate)).toBe("var(--primary-color)");
      expect(resolveColorFromStops(50, [], interpolate)).toBe("var(--primary-color)");
    });
    ...
    it("returns color of first stop if progress is less than first percent", () => {
      ...
      expect(resolveColorFromStops(10, stops, interpolate)).toBe("var(--red-color)");
    });
    ...
    it("returns color of last stop if progress is greater than last percent", () => {
      ...
      expect(resolveColorFromStops(90, stops, interpolate)).toBe("var(--blue-color)");
    });
    ...
    it("returns exact match if progress equals stop percent", () => {
      ...
      expect(resolveColorFromStops(50, stops, interpolate)).toBe("var(--green-color)");
    });
    ...
    it("returns default if resolved color is invalid", () => {
      ...
      expect(resolveColorFromStops(50, stops, interpolate)).toBe("var(--primary-color)");
    });
  });
  ...
  describe("interpolated colors", () => {
    ...
    ])("interpolates between %s (%s) and %s (%s) at %s%%", (p1, c1, p2, c2, value, expected) => {
      hass.getState.mockImplementation((key) => {
        const map = {
          "sensor.lower": c1,
          "sensor.upper": c2,
...
describe("interpolated with multiple stops", () => {
  test.each([
    [25, "red", 50, "yellow", 75, "green", 30, "color-mix(in srgb, var(--red-color) 80%, var(--yellow-color) 20%)"],
    [25, "red", 50, "yellow", 75, "green", 50, "var(--yellow-color)"],
    [25, "red", 50, "yellow", 75, "green", 60, "color-mix(in srgb, var(--yellow-color) 60%, var(--green-color) 40%)"],
    [25, "red", 50, "yellow", 75, "green", 74, "color-mix(in srgb, var(--yellow-color) 4%, var(--green-color) 96%)"],
  ])("interpolates across multiple stops with value %s", (p1, c1, p2, c2, p3, c3, value, expected) => {
    hass.getState.mockImplementation((key) => {
      const map = {
        "sensor.stop1": c1,
        "sensor.stop2": c2,
